...
input_dict, y_train, input_dict_test, y_test, cardinality, weights_train = df_to_tf(
    df, cat_feats, useless_cols, use_validation=True
)
...
def fitness(
    ...
    early_stopping = tfk.callbacks.EarlyStopping(
        "val_root_mean_squared_error", patience=15, verbose=0, restore_best_weights=True
    )
    ...
    if rmse < best_rmse:
        ...
        res = (
            ...
            - model.predict(input_dict_test, batch_size=10000).flatten()
        )
...
checkpoint_callback = CheckpointSaver(
    (os.path.join(MODELS_PATH, "%s_%s_checkpoint.pkl" % (horizon, task)))
)
global best_rmse
best_rmse = 100
try:

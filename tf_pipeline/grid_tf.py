...
input_dict, y_train, input_dict_test, y_test, cardinality, weights_train = df_to_tf(df,
                                                                                    use_validation=True)


# search space for the MLP
...
dim_emb_dim = Integer(low=30, high=80, name='emb_dim')
...
path_checkpoint = (os.path.join(MODELS_PATH, "%s_%s_other_checkpoint.pkl" % (horizon, task)))
checkpoint_callback = CheckpointSaver(path_checkpoint)
global best_rmse
best_rmse=100
try:    
    res = load(path_checkpoint)
    x0 = res.x_iters
    y0 = res.func_vals
    best_rmse = min(y0)

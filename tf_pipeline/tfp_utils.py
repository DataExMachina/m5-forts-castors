...
kl_divergence_function = (
    lambda q, p, _: tfd.kl_divergence(q, p) / kl_weight  # pylint: disable=g-long-lambda
)  # KL over the batch of 2048 (should normalize by the number of mini batch)


def create_mlp(layers_list=[512, 512, 512, 64], type_output="poisson"):
    """
    description : 
    ...
    if len(num_feats) > 1:
        for num_var in num_feats:
            input_num = tfkl.Input(shape=(1,), name="input_{0}".format(num_var))
        ...
        input_num = tfkl.Input(shape=(1,), name="input_{0}".format(numeric_features[0]))
        inputs.append(input_num)
    ...
    for categorical_var in cat_feats:
        ...
        # functionnal loop
        input_cat = tfkl.Input(shape=(1,), name="input_{0}".format(categorical_var))
        inputs.append(input_cat)
        embedding = tfkl.Embedding(
            vocab,
            embedding_size,
            embeddings_regularizer=tf.keras.regularizers.l1(1e-8),
            name="embedding_{0}".format(categorical_var),
        )(input_cat)
        embedding = tfkl.Dropout(0.1)(embedding)
        vec = tfkl.Flatten(name="flatten_{0}".format(categorical_var))(embedding)
    ...
    for i in range(len(layers_list)):
        dense = tfkl.Dense(
            layers_list[i], name="Dense_{0}".format(str(i)), activation="elu"
        )(dense)
        dense = tfkl.Dropout(0.1)(dense)
        dense = tfkl.BatchNormalization()(dense)

    # lognormal
    if type_output == "gaussian":
        dense2 = tfk.layers.Dense(2, activation="softplus", name="Output")(dense)

        output = tfp.layers.DistributionLambda(
            lambda t: tfd.Normal(t[..., 0], scale=t[..., 1:])
        )(dense2)

    # Poisson
    elif type_output == "poisson":
        dense2 = tfk.layers.Dense(1, name="Output")(dense)
        output = tfp.layers.DistributionLambda(
            lambda t: tfd.Poisson(rate=tf.math.softplus(t[..., 0]))
        )(dense2)

    # Gamma
    elif type_output == "gamma":
        dense2 = tfk.layers.Dense(2, name="Output", activation="softplus",)(dense)

        output = tfp.layers.DistributionLambda(
            lambda t: tfd.Gamma(concentration=0.01 * t[..., 0], rate=0.01 * t[..., 1])
        )(dense2)
    else:
        output = tfk.Dense(1)
    ...
    model.compile(loss=neg_log_likelihood_continuous, optimizer=opt, metrics=[rmse])
    return model
...
def create_mlp_full_bayes(layers_list=[512, 256, 128, 64], type_output="poisson"):
    """
    description : 
    generate regression mlp with
    both embedding entries for categorical features and 
    ...
    for i in range(len(layers_list)):
        dense = tfp.layers.DenseVariational(
            layers_list[i],
            activation="elu",
    ...
    if type_output == "gaussian":
        dense2 = tfp.layers.DenseVariational(
            2,
            # activation = 'softplus',
            make_posterior_fn=posterior_mean_field,
            make_prior_fn=prior_trainable,
    ...
    # Gamma
        ...
        output = tfp.layers.DistributionLambda(
            lambda t: tfd.Gamma(concentration=t[..., 0], rate=t[..., 1])
        )(dense2)

    else:
        output = tfk.Dense(
            1, kernel_divergence_fn=kl_divergence_function, name="Output"
        )(dense)
    ...
    model.compile(
        loss=neg_log_likelihood_continuous,
        optimizer=opt,

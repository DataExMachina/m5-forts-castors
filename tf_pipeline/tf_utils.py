...
def poisson(y_true, y_pred):
    ...
    """
    return K.mean(
        K.maximum(0.0, y_pred) - y_true * K.log(K.maximum(0.0, y_pred) + K.epsilon()),
        axis=-1,
    )
...
def tweedie_loss(y_true, y_pred):
    ...
    p = 1.5
    dev = (
        K.pow(y_true, 2 - p) / ((1 - p) * (2 - p))
        - y_true * K.pow(K.maximum(0.0, y_pred) + K.epsilon(), 1 - p) / (1 - p)
        + K.pow(K.maximum(0.0, y_pred) + K.epsilon(), 2 - p) / (2 - p)
    )
...
def create_mlp(
    ...
    if len(num_feats) > 1:
        ...
        input_num = tfkl.Input(shape=(1,), name="input_{0}".format(num_feats[0]))
        inputs.append(input_num)
    ...
    if loss_fn == "poisson":
        ...
        raise ValueError("Loss function should be either Poisson or tweedie for now")
    return model
...
def train_mlp(horizon="validation", task="volume", training_params=None):
    ...
    num_feats = df.columns[~df.columns.isin(useless_cols + cat_feats)].to_list()
    input_dict, y_train, cardinality, weights_train = df_to_tf(
        df, cat_feats, useless_cols, use_validation=False
    )
...
>>>>>>> parent of e6adfb9... nouveau pipeline
    ...
    early_stopping = tfk.callbacks.EarlyStopping(
        "root_mean_squared_error", patience=10, verbose=0, restore_best_weights=True
    )

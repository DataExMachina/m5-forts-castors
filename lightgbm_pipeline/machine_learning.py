...
def train(horizon="validation", task="volume"):
    ...
    df = pd.read_parquet(os.path.join(REFINED_PATH, '%s_%s_fe.parquet' % (horizon, task)))
...
def create_dt(horizon="validation", tr_last=1913):
    ...
    dt = pd.read_csv(os.path.join(RAW_PATH, "sales_train_%s.csv" % horizon), 
...
def predict(horizon="validation", task="volume"):
    ...
    m_lgb = lgb.Booster(model_file=os.path.join(MODELS_PATH, '%s_%s_lgb.txt' % (horizon, task)))
    ...
    for i in range(0, 28):
        ...
        tst = dataframe[(dataframe.date >= day - timedelta(days=366)) & (dataframe.date <= day)].copy()
        ...
        if task=="volume":
            dataframe.loc[dataframe.date == day, "sales"] = tst['rate']*m_lgb.predict(tst[train_cols])
        elif task=="share":
            dataframe.loc[te.date == day, "sales"] = m_lgb.predict(tst[train_cols])
            shares = dataframe.groupby(['dept_id', 'store_id', 'date'])['sales'].sum().reset_index().rename(columns={'sales': 'gp_sales'})
    ...
    if horizon=="validation":
        ...
        te_sub.loc[te.date >= fday + timedelta(days=h), "id"] = te_sub.loc[te.date >= fday+timedelta(days=h), "id"].str.replace("evaluation", "validation")
    ...
    if task=="volume":
        te_sub.to_csv(os.path.join(SUBMIT_PATH, "lgb_estim_%s.csv" % horizon), index=False)
    else:
        te_sub.to_csv(os.path.join(EXTERNAL_PATH, "lgb_weights_%s.csv" % horizon), index=False)
